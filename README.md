...
HIGH_SOCIETY_CATALOG_URL="https://highsocietymn.vercel.app/api/products"
```
...
Deployments generate the Prisma client but never mutate the database. Run `npm run db:push` deliberately when applying a reviewed schema update. The core search experience falls back to the public High Society catalog when the independent catalog has no listings; override that read-only source with `HIGH_SOCIETY_CATALOG_URL` if it moves.
